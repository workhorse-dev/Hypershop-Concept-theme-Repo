...
console.log(
  theme.settings.themeName +
    ...
    ") by RoarTheme | Learn more at https://roartheme.co"
);
  ...
  theme.utils = {
    ...
    setScrollbarWidth: () => {
      ...
      document.documentElement.style.setProperty(
        "--scrollbar-width",
        `${scrollbarWidth > 0 ? scrollbarWidth : 0}px`
      );
    },
    ...
    Updater: (function () {
      class Updater {
        ...
        update(targetElement) {
          ...
          let cssText = `--t: ${Math.floor(scrollTop) > 0 ? 1 : 0}; --b: ${
    ...
    class Animation {
      ...
      get delay() {
        return this.container.hasAttribute("data-animate-delay")
          ? parseInt(this.container.getAttribute("data-animate-delay")) / 1000
          : 0;
      }
...
class StickyHeader extends CustomHeader {
  ...
  onScrollHandler() {
    ...
    if (
      scrollTop >
      this.headerBounds.top + this.firstScrollTop + this.headerBounds.height
    ) {
...
class QuantityInput extends HTMLElement {
  ...
  updateQuantityRules({ data: { sectionId, productId, parsedHTML } }) {
    ...
    const quantityForm = this.closest(
      `#QuantityForm-${sectionId}-${this.productId}`
    );
...
customElements.define("api-hover-button", APIHoverButton, {
  extends: "button",
});
...
customElements.define("api-magnet-button", APIMagnetButton, {
  extends: "button",
});
...
class StickyElement extends HTMLElement {
  ...
  get headerHeight() {
    ...
    return header
      ? Math.round(header.clientHeight) + 20
...
class MotionList extends HTMLElement {
  ...
  load() {
    Motion.animate(
      ...
      {
        duration: theme.config.motionReduced ? 0 : 0.5,
...
class GMap extends HTMLElement {
  ...
  loadScript() {
    return new Promise((resolve, reject) => {
      ...
      script.async = true;
      script.src = `https://maps.googleapis.com/maps/api/js?key=${this.getAttribute(
        "data-api-key"
      )}&callback=gmNoop`;
    });
  }
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    this.buttons.forEach((button, index) => {
      ...
      button
        ...
        ?.classList.toggle("button--secondary", index !== parseInt(newValue));
    });
...
class ButtonController {
  ...
  // find the element this button controls (via aria-controls)
  get controlledElement() {
    return this.button.hasAttribute("aria-controls")
      ? document.getElementById(this.button.getAttribute("aria-controls"))
      : null;
  }

  // add listeners (click + slider status updates)
  load() {
    this.onClickListener = this.onClick.bind(this);
    this.button.addEventListener("click", this.onClickListener);

    const eventName =
      this.type === "previous" ? "slider:previousStatus" : "slider:nextStatus";
    if (this.controlledElement) {
      this.updateStatusListener = this.updateStatus.bind(this);
      this.controlledElement.addEventListener(
        eventName,
  ...
  // remove listeners if needed (optional cleanup)
  unload() {
  ...
  // when the button is clicked → send slider event
  onClick() {
    const eventName =
      this.type === "previous" ? "slider:previous" : "slider:next";
    (this.controlledElement ?? this.button).dispatchEvent(
      new CustomEvent(eventName, { bubbles: true, cancelable: true })
    );
  }

  // when slider sends status → update button state
  updateStatus(event) {
    switch (event.detail.status) {
      case "hidden":
...
document.addEventListener("DOMContentLoaded", () => {
  document
    ...
    .forEach((btn) => new ButtonController(btn, "previous"));
  document
    .querySelectorAll(".next-button")
    .forEach((btn) => new ButtonController(btn, "next"));
  document.querySelectorAll(".button.button--secondary").forEach((btn) => {
    if (btn.querySelector("[data-fill]")) {
      const hover = new theme.HoverButton(btn);
      hover.load();
    }
  });
});
...
class VariantSelects extends HTMLElement {
  ...
  get selectedOptionValues() {
    ...
    ).map((selector) => selector.getAttribute("data-option-value-id"));
  }
}
...
class ProductInfo extends HTMLElement {
    ...
    fetch(requestUrl, { signal: this.abortController.signal })
      ...
      .then(() => {
        ...
        if (activeElement.hasAttribute("align-selected")) {
          const scrollElement = activeElement.closest(
            activeElement.getAttribute("align-selected")
          );
          scrollElement.scrollTo({
  ...
  updateOptionValues(parsedHTML) {
    const variantSelects = parsedHTML.getElementById(
...
class ProductForm extends HTMLFormElement {
  ...
  get bundles() {
    return Array.from(
      document.querySelectorAll(
        `[aria-controls="${this.getAttribute(
          "id"
...
class ProductStickyForm extends HTMLElement {
  ...
  generateSrcset(image) {
    const widths = this.productMedia
...
class ProductBundleDetails extends AccordionDetails {
  ...
  onCartUpdate(event) {
    if (event.source === "product-form") {
      Array.from(
        this.querySelectorAll('input[name="bundles"]:checked')
      ).forEach((input) => {
...
class MediaGallery extends HTMLElement {
  ...
  get gangOption() {
    ...
    return mediaItemWithGang
      ? (this._gangOption = mediaItemWithGang.getAttribute("data-gang-option"))
      : false;
  }
  ...
  onVariantChanged(event) {
    ...
    if (this.gangOption) {
      this.sliderGallery.items.forEach(
        (item) =>
          (item.hidden =
            item.getAttribute("data-gang-connect") !==
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if (
      ...
      if (fromElement.getAttribute("data-type") === "video") {
        const fromVideoElement = theme.utils.visibleMedia(
      ...
      if (toElement.getAttribute("data-type") === "video") {
        const toVideoElement = theme.utils.visibleMedia(
...
class ControlButtonController {
  ...
  get paused() {
    return this.button.hasAttribute("paused");
  }
  ...
  load() {
    ...
    // handle attribute changes if someone updates paused state manually
    const observer = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.attributeName === "paused") {
          if (this.paused) {
            (this.controlledElement ?? this.button).dispatchEvent(
              new CustomEvent("slider:pause", { bubbles: true })
            );
...
document.addEventListener("DOMContentLoaded", () => {
  document
    ...
    .forEach((btn) => new ControlButtonController(btn));
});
...
class FiltersToggle extends HoverButton {
  constructor() {
    ...
    if (theme.config.hasLocalStorage) {
      const expanded =
        window.localStorage.getItem(
          `${theme.settings.themeName}:filters-toogle`
        ) || "false";
...
class ProductBundleInfo extends ProductInfo {
  ...
  getSelectedVariant(productInfoNode) {
    const selectedVariant = productInfoNode.querySelector(
      `product-bundle-info[data-product-id="${this.productId}"] [data-selected-variant]`
    )?.textContent;
    return !!selectedVariant ? JSON.parse(selectedVariant) : null;
  }
}
...
class ProductBundle extends HTMLElement {
  ...
  get bundleMax() {
    ...
    const maximum = this.hasAttribute("data-maximum")
      ? parseInt(this.getAttribute("data-maximum"))
      : 3;
  ...
  addVariant(product, variant) {
    ...
    if (variantElement) {
      ...
      const media = variantElement.querySelector(
        "[data-product-bundle-variant-media]"
      );
      if (preview_image.length > 0) {
        media.innerHTML = `<img src="${this.getResizedImageSrc(
          preview_image,
          180
  ...
  clearVariant(variant) {
    ...
    variant.querySelector("[data-product-bundle-variant-content]").innerHTML =
      ...
      if (index !== 0 && offsetTop < 0) {
        ...
        if (Math.abs(offsetTop) > mediaScrollTop) {
          ...
          if (controlledElement) {
            if (progress === 100)
              controlledElement.classList.add(
                "opacity-0",
                "pointer-events-none"
              );
            if (progress === 0)
